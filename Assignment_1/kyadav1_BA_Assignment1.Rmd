...
Online_Retail_2<-subset(Online_Retail[,c(1,5,7,10)])
Online_Retail_2 <- na.omit(Online_Retail_2) 

 #Aggregating the data based on Customer ID and determining the distinct Invoice date for each customer id's (to reduce the calculation)
...
# Changing the type 
Difference_InvoiceDt<-as.numeric(Difference_InvoiceDt)
...
##Note: After the merging the difference of the invoice dates and customer id's, we can see that since we have not done groupby using customer id's for it ,the first row in each customer id category has some values therefore:-
 
# Updating the first record for  difference in dates each customer as 0
 InvoiceDt_byCustomerID <- InvoiceDt_byCustomerID %>%
                          group_by(CustomerID) %>%
                          mutate(X.Difference_InvoiceDt. = replace ( X.Difference_InvoiceDt. , row_number() == 1, 0))
